...
// [ALLbOokS] Base dos pesos é configurável: por padrão o layout do demo original
// (./onnx/<lang>), mas a app passa a URL do Hugging Face na mensagem `load`, pra
// que o download dos ~155MB não passe pelo servidor.
let bundleBase = "./onnx";

function bundleDir(language) {
    return `${bundleBase.replace(/\/+$/, "")}/${language}`;
}
...
// [ALLbOokS] carrega o encoder na primeira clonagem de voz.
async function ensureEncoderSession() {
    if (mimiEncoderSession) {
        return mimiEncoderSession;
    }
    postMessage({ type: "status", status: "Baixando encoder de voz...", state: "loading" });
...
async function loadBundle(language, { initialLoad = false } = {}) {
    ...
    // [ALLbOokS] o mimi_encoder só serve pra CLONAR voz a partir de um áudio.
    // Quem usa as vozes prontas não precisa dele, então ele é carregado sob
    // demanda em ensureEncoderSession() — 21MB a menos no primeiro acesso.
